...
# 摄像头相关参数设置
Lower = np.array([20, 70, 140])  # 设置颜色二值化 目前用黄色测试
...
producer_mutex = Condition()  # 锁，控制队列访问
consumer_mutex = Condition()  # 锁，控制队列访问
queue_num = 7
...
def process_image(frame):
    ...
    img_border = cv2.copyMakeBorder(img_erode, 1, 1, 1, 1, cv2.BORDER_CONSTANT, value=BLACK)
    ...
    if max_cnt_area / screen_area > detect_threshold:
        if cy <= 320:
            ...
            v_int = k * distance_int * distance_int
            v_info = v_int & 0x00ff

            str_info = [0x55, 0x00, v_info, 0xff]
            return str_info
            ...
            str_info = [0x55, 0x01, v_info, 0xff]
            return str_info
...
def consumer(name, strinfo_queue):
    ...
    while True:
        ...
        str_info_list[2] = str_info_list[1]
        str_info_list[1] = str_info_list[0]
        str_info_list[0] = str_info[2]
        blur_v = str_info_list[0]*0.5 + str_info_list[1]*0.3 + str_info_list[2]*0.2
        v_int = int(blur_v)
        if v_int <= 20:
            v_int = 0
        v_info = v_int & 0x00ff
